...
???
- Taking the full results from that calulcation we can make a visual table and have a look at what each scagnostic is looking for
...
- if you are paying close attention you may have noticed that the scagnostics based on the MST are the ones that most freuently have only 1 or two plot, this is because distributions are very condensed 
...
```
???
- Here is the same visual table but we are only plotting clumpy with clumpy2
...
# make SPLOM
p <- GGally::ggpairs(AFLW_splom, columns=c(3,6,10,14,15),
                     aes(label=lab)) +
  theme_classic()
ggplotly(p)
```
...
- you want to make the label the variable names, and use an interactive plot to find the scatter plots that are interesting
...
???
- Now we look for plots that are different to the rest on the scagnostics
---
# AFLW Example: Quiz!
...
???
- since they are supposed to be directly comparable on a 0 to 1 scale, if one scagnostic appears a lot, it is likely an underlying structure through the whole data set
...
???
- while top_gives() gives you the scatter plot that had the highest value on that scatter plot for each scagnostic
- a reccuring scatter plot that is high on a lot of measures is likely an outlier scatter plot that has an interesting structure
...
df <- as_tibble(mydat) %>% 
  mutate(species = rep(feature_vs_noise$group, d[3]))
dfaxes <- as_tibble(myaxes) %>%
  mutate(labels=rep(colnames(feature_vs_noise[,1:5]), d[3]))
dfaxes_mat <- dfaxes %>%
  mutate(xloc = rep(max(df$d1)+0.4, d[3]*5), yloc=rep(seq(-0.2, 0.2, 0.1), d[3]), 
...
- We have done a bit more work into using the scagnostics as a projection pursuit measure in the tour package
- this is an example of a guided tour that is trying to find the projection that maximises the difference in the convex scagnostic between the two groups
